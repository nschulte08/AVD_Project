...
%--------------------------------------------------------------------------
M_cr_sub = 0.95;             % Subsonic cruise altitude, (m)
M_cr_super = 1.4;            % Supersonic cruise altitude, (m)
...
%--------------------------------------------------------------------------
% aero:
AoA_TO = 1.0; % Placeholder, need to find from CLa and CL from aerofunk_lift
CL_TO = 2*MTOW/(0.5*rho_TO*V_TO^2*Sref);   % N/N
[CD_TO, CD0_TO] = aerofunk_drag(alt_TO, M_TO, Sref, CL_TO, AoA_TO, AR, TR);

%--------------------------------------------------------------------------
% performance:
[V_stall, V_TO, S_G, S_TO, BFL] = perf_takeoff(ne, V_TO, T_max, alt_TO, MTOW, Sref, CL_max, CD0_TO, K);

%--------------------------------------------------------------------------
% S&C:
[CMa, Cl_beta, Cn_beta, Cn_dr, S_VT, l_VT] = SC_derivatives(M_TO, AR, 24, Sref, b, TR, CL_TO, 'takeoff');
...
[CD_climb, CD0_climb] = aerofunk_drag(alt_climb, M_climb, Sref, CL_climb, AoA_climb, AR, TR);
...
AoA_cr = 1.0;     %placeholder, need to calculate alpha reqd based on CLa and cruise CL
%LD_max = 0.5*sqrt((pi*AR*e)/CD0_sub);                 

[R_constH, R_CC, TOF_constH, TOF_CC] = perf_cruise(M_cr, alt_cr, W_cruise_start, W_cruise_end, Sref, AoA_cr, AR, TSFC, TR);

%--------------------------------------------------------------------------
% S&C:
[CMa, Cl_beta, Cn_beta, Cn_dr, S_VT, l_VT] = SC_derivatives(M_cr, AR, 24, Sref, b, TR, CL_cr, 'cruise');
...
[S_land, FAR_land] = perf_land(M_land, alt_land, Sref, AR_low, W_land, CL_max, T_max, TR);

%--------------------------------------------------------------------------
% S&C:
[CMa, Cl_beta, Cn_beta, Cn_dr, S_VT, l_VT] = SC_derivatives(M_land, AR, 24, Sref, b, TR, CL_max, 'landing');
