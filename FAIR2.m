...
% options = optimoptions('fsolve','OptimalityTolerance', 1e-1);
% T = fsolve(@(T) hfun_vit(T,h),500,options);
h_poly = [A_vit(8)/8,A_vit(7)/7,A_vit(6)/6,A_vit(5)/5,A_vit(4)/4,A_vit(3)/3,A_vit(2)/2,A_vit(1),href_vit-h];
T_roots = roots(h_poly);
T = real(T_roots(real(T_roots)>=0));
