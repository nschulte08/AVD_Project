...
[T,~,exitflag] = fsolve(@(T) phifun_combined(T,phi_f,f),500,optimoptions('fsolve','Display','off','MaxIterations',50));


cp_pure = A_pure(1)+A_pure(2)*T+A_pure(3)*T.^2+A_pure(4)*T.^3+A_pure(5)*T.^4+A_pure(6)*T.^5+A_pure(7)*T.^6+A_pure(8)*T.^7;%[Btu/(lbm ? °R)]
cp_vit = A_vit(1)+A_vit(2)*T+A_vit(3)*T.^2+A_vit(4)*T.^3+A_vit(5)*T.^4+A_vit(6)*T.^5+A_vit(7)*T.^6+A_vit(8)*T.^7;%[Btu/(lbm ? °R)]
cp = (cp_pure+f*cp_vit)/(1+f);%[Btu/(lbm ? °R)]
h_pure = href_pure + A_pure(1)*T+A_pure(2)/2*T.^2+A_pure(3)/3*T.^3+A_pure(4)/4*T.^4+A_pure(5)/5*T.^5+A_pure(6)/6*T.^6+A_pure(7)/7*T.^7+A_pure(8)/8*T.^8;%pg 91 "Elements of Propulsion"[Btu/lbm]
h_vit = href_vit + A_vit(1)*T+A_vit(2)/2*T.^2+A_vit(3)/3*T.^3+A_vit(4)/4*T.^4+A_vit(5)/5*T.^5+A_vit(6)/6*T.^6+A_vit(7)/7*T.^7+A_vit(8)/8*T.^8;%pg 91 "Elements of Propulsion"[Btu/lbm]
