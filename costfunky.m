function [ RTDE_Cost, ] = costfunky( W_e, V_max, T_sl, M_max, W_to, W_f, Range, t_cl, t_ds, t_cr, W_A, N_e )
...
~t_cl = 1;               % PLACEHOLDER Climb time [hr]
~t_cr = 4;               % PLACEHOLDER Cruise time [hr]
~t_de = 1;               % PLACEHOLDER Descent time [hr]
...
V_bl = R/t_bl;                      % Total block flight velocity [nmi/h] {1 - 71}
...
% Analysis
C_crew = ((1 + K1)/V_bl)*(SAL1/AH1) + TEF1/V_bl +...
         ((1 + K2)/V_bl)*(SAL2/AH2) + TEF2/V_bl;    % Crew cost (Assuming 2 crew) [USD/nmi] {1}
...
% Analysis
C_pol = 1.05*(W_f/R)*(FP/FD);   % Fuel and oil cost, assuming oil is 5% of overall...
                                 %POL costs [USD/nmi] {1}

% Airframe Insurance Cost
%-----------------------------------------------------------------------------------------------%
% Inputs
f_ins_hull = [0.005 0.03];  % PLACEHOLDER Annual hull insurance rate...
                             %[USD/aircraft price/aircraft/year] {1}
...
DOC_fly_min = min(C_crew) + min(C_pol) + min(C_ins);    % Minimum direct operating cost...
                                                         %of flying [USD/nmi]
DOC_fly_max = max(C_crew) + max(C_pol) + max(C_ins);    % Maximum direct operating cost...
                                                         %of flying [USD/nmi]
...
% Airframe and Non-Engine-Systems Maintenance
 ...
 MHR_flt_lab = 3.0;         % PLACEHOLDER Airframe and systems mainteance man hours per...
                             %flight hour {1}
 ...
 MHR_bl_lab = MHR_flt_lab*(t_flt/t_bl);     % Airframe and systems mainteance man hours per...
                                             %block hour {1}

 C_lab_ap = 1.03*MHR_bl_lab*R_l_ap/V_bl;    % Airframe and non-engine-systems maintenance...
                                             %labor costs [USD/nmi] {1}

% Engine maintenance
 % Analysis

 MHR_bl_eng = 3.0*0.067*W_A/1000;                   % Engine maintenance hours per block hour...
                                                     %per engine {1}
 R_l_eng = [5.00 18.40];                            % Engine maintenance labor rate [USD/hr] {1}
 C_lab_eng = 1.03*1.3*N_e*MHR_bl_eng*R_l_eng/V_bl;  % Engine maintenance labor costs...
                                                     %[USD/nmi] {1}
...
% Airframe and Non-Engine-Systems Materials Maintenance
 ...
 C_mat_ap_bl = 300;     % PLACEHOLDER Airframe and non-engine-systems maintenance material...
                         %cost per block hour [USD/hr] {1}
 
 % Analysis
 
 C_mat_ap = 1.03*C_mat_ap_bl/V_bl;  % Airframe and non-engine-systems materials maintenance...
                                     %costs [USD/nmi] {1}
...
% Engine Materials Maintenance
 ...
 C_mat_eng_bl = (5.43e-5*EP*ESPPF - 0.47)./K_H_em;  % Engine maintenance material cost per...
                                                     %block hour per engine [USD/hr] {1}
 C_mat_eng = 1.03*1.3*N_e*C_mat_eng_bl/V_bl;        % Engine materials maintenance costs...
                                                     %[USD/nmi] {1}
...
% Applied Maintenance Burden
 ...
 f_amb_lab = [.9 1];    % Overhead distribution factor for labor cost {1}
 f_amb_mat = [.3 .4];   % Overhead distribution factor for material cost {1}
 
 % Analysis 
 
 C_amb = 1.03*(f_amb_lab.*(MHR_bl_lab*R_l_ap +...
         N_e*MHR_bl_eng*R_l_eng) + f_amb_mat.*(C_mat_ap_bl...
         + N_e*C_mat_eng_bl))/V_bl;                             % Applied maintenance burden...
                                                                 %[USD/nmi] {1}

% Final Maintenance DOC Analysis 
 
DOC_maint_min = min(C_lab_ap) + min(C_lab_eng) +...
                min(C_mat_ap) + min(C_mat_eng) + min(C_amb);    % Minimum direct operating...
                                                                 %cost of maintenance...
                                                                 %[USD/nmi] {1}
DOC_maint_max = max(C_lab_ap) + max(C_lab_eng) +...
                max(C_mat_ap) + max(C_mat_eng) + max(C_amb);    % Maximum direct operating...
                                                                 %cost of maintenance...
                                                                 %[USD/nmi] {1}
...
% Airframe Depreciation Cost
 ...
 C_dap = F_dap*(AMP - N_e*EP - ASP)./(DP_ap*U_ann*V_bl);    % Cost of airframe depreciation...
                                                             %[USD/nmi] {1}
...
% Space Parts Depreciation
 ...
 C_dapsp = (F_dapsp*F_apsp*(AMP - N_e*EP))./(DP_apsp*U_ann*V_bl);   % Cost of spare parts...
                                                                     %depreciation...
                                                                     %[USD/nmi] {1 - p105}
...
% Engine Spare Parts Depreciation
 ...
 C_dengsp = (F_dengsp*F_engsp*N_e*EP*ESPPF)./...
            (DP_engsp*U_ann*V_bl);                  % Cost of engine spare parts...
                                                     %depreciation [USD/nmi] {1 - p106}

% Final Depreciation DOC Analysis 

DOC_depr_min = min(C_dap) + min(C_deng) +...
               min(C_dav) + min(C_dapsp) + min(C_dengsp);   % Minimum direct operating cost...
                                                             %of depreciation [USD/nmi]
DOC_depr_max = max(C_dap) + max(C_deng) +... 
               max(C_dav) + max(C_dapsp) + max(C_dengsp);   % Maximum direct operating cost...
                                                             %of depreciation [USD/nmi]
...
DOC_int0 = DOC_fly + DOC_maint + DOC_depr;  % International Direct operating cost...
                                             %[USD/nmi] {1 - p80}
...
% Navigation Fees
 ...
 C_apnf_dom = 0;    % Domestic (USA) flight navigation fee per aircraft per flight...
                     %[USD] {1 - p109}
 C_apnf_int = 10;   % International flight navigation fee per aircraft per flight...
                     %[USD] {1 - p109}

 % Analysis
 
 C_nf_dom = C_apnf_dom/(V_bl*t_bl);     % Cost of domestic navigation fee [USD/nmi] {1 - p108}
 C_nf_int = C_apnf_int/(V_bl*t_bl);     % Cost of international navigation fee [USD/nmi]...
                                         %{1 - p108}
...
DOC_lnr_dom_min = min(C_lf) + min(C_nf_dom)...
                + min(C_rt_dom);                % Minimum domestic direct operating cost of...
                                                 %landing fees [USD/nmi] {1 - p107}
DOC_lnr_dom_max = max(C_lf) + max(C_nf_dom)...
                + max(C_rt_dom);                % Maximum domestic direct operating cost of...
                                                 %landing fees [USD/nmi] {1 - p107}

DOC_lnr_dom = [DOC_lnr_dom_min DOC_lnr_dom_max];

DOC_lnr_int_min = min(C_lf) + min(C_nf_int)...
                + min(C_rt_int);                % Minimum international direct operating cost...
                                                 %of landing fees [USD/nmi] {1 - p107}
DOC_lnr_int_max = max(C_lf) + max(C_nf_int)...
                + max(C_rt_int);                % Maximum international direct operating cost...
                                                 %of landing fees [USD/nmi] {1 - p107}
...
DOC_dom = DOC_fly + DOC_maint + DOC_depr...
        + DOC_lnr_dom + DOC_fin_dom;            % Domestic Direct operating cost...
                                                 %[USD/nmi] {1 - p80}
DOC_int = DOC_fly + DOC_maint + DOC_depr...
        + DOC_lnr_int + DOC_fin_int;            % International Direct operating cost...
                                                 %[USD/nmi] {1 - p80}
