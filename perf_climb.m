function [ROC, gamma_climb, S_climb, dt_climb] = perf_climb(CD_climb, V_stall, V_sub, rho_TO, T_max, W_climb, alt_climb, V_climb)
%This function outputs the performance characteristics of the OFW during
%the climb phase

%Inputs:
%CD_climb:          Climb drag coefficient
%V_stall:           Stall velocity (m/s)
%V_sub_R:           Subsonic velocity (m/s)
%rho_TO:            Takeoff density (kg/m^3)
...
%S_climb_sub:       Distance covered when climbing to subsonic cruise altitude (m)
%S_climb_super:     Distance covered when climbing to supersonic cruise altitude (m)
%dt_climb_sub:      Time of flight during climb when climbing to subsonic cruise altitude
%dt_climb_super:    TOF when climbing to supersonic cruise altitude
...
V = linspace(V_stall,V_sub)';                    % [m/s] veloctiy vector
